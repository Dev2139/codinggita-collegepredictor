...
const PredictedCollegesTable = ({ data = [], exam = "" }) => {
  ...
  const renderTableHeader = () => (
    <tr className={commonHeaderClass}>
      {predicted_colleges_table_column.map((column) => (
        <th key={column.key} className="p-2 border-r border-[#facc15]"> {/* Yellow border */}
          {column.label}
        </th>
      ))}
    </tr>
      ...
      return (
        <tr
          ...
          className={`${commonCellClass} ${
            index % 2 === 0 ? "bg-[#facc15]" : "bg-white" // Alternating yellow and white rows
          }`}
        >
          {exam !== "TNEA" && (
            <td className="p-2 border-r border-[#9333ea]"> {/* Purple border */}
              {transformedItem.state || "N/A"}
            </td>
          )}
          {exam === "TNEA" && (
            <td className="p-2 border-r border-[#9333ea]">
  ...
  const renderLegend = () => {
    ...
    return (
      <div className="flex flex-col items-center text-sm sm:text-base mb-4 text-[#9333ea]"> {/* Purple text */}
  ...
  return (
    <div className="w-full mx-auto overflow-x-auto">
      {renderLegend()}
      <table className={`${commonTableClass} border border-[#9333ea]`}> {/* Purple table border */}
